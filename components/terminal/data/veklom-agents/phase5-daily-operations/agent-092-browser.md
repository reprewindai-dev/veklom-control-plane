# Agent-092 — BROWSER: Marketplace & Purchase (Hands)
...
Automate and test the flow: browse marketplace → view listing → purchase → download → verify access
...
```python
# File: tests/browser/test_092.py
...
def test_flow():
    with sync_playwright() as p:
        ...
        # [Execute flow: browse marketplace → view listing → purchase → download → verify access]
        # Screenshot each major step:
        page.screenshot(path="evidence/agent-092-step1.png")
...
Screenshot after every step. Name format: `agent-092-{step}-{timestamp}.png`
Save to `playwright-report/agent-092/`
